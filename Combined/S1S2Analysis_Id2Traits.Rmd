...
```{r}
dm1<-lmer(scale(selfResp) ~ scale(order) + scale(subTend) + scale(traitTend)  + ( scale(order) | subID ) + ( 1 | traits), data=orderDf1)
summary(dm1)
dm1.plot <- ggpredict(dm1, c("order")) %>% plot(show.title=FALSE) + jtools::theme_apa() + xlab("Distance from Identity") + ylab("Self-Evaluation")
dm1.plot
```
...
```{r}
dm2<-lmer(scale(selfResp) ~ scale(order) + scale(subTend) + scale(traitTend)  + ( scale(order) | subID ) + ( 1 | traits), data=orderDf2)
summary(dm2)
dm2.plot <- ggpredict(dm2, c("order")) %>% plot(show.title=FALSE) + jtools::theme_apa() + xlab("Distance from Identity") + ylab("Self-Evaluation")
dm2.plot
```

# Trait Distance Moderated by Identification

## Study 1

```{r}
dms1<-lmer(scale(selfResp) ~ scale(order) * scale(streng) + scale(subTend) + scale(traitTend)  + ( scale(order) | subID ) + ( 1 | traits), data=orderDf1)
summary(dms1)
dms1.plot <- ggpredict(dms1, c("order", "streng")) %>% plot(show.title=FALSE) + jtools::theme_apa() + xlab("Distance from Identity") + ylab("Self-Evaluation")
dms1.plot
```
...
```{r}
dms2<-lmer(scale(selfResp) ~ scale(order) * scale(streng) + scale(subTend) + scale(traitTend)  + ( scale(order) | subID ) + ( 1 | traits), data=orderDf2)
summary(dms2)
dms2.plot <- ggpredict(dms2, c("order", "streng")) %>% plot(show.title=FALSE) + jtools::theme_apa() + xlab("Distance from Identity") + ylab("Self-Evaluation")
dms2.plot
```
